...
const StepEmail = ({ name, setName, email, setEmail, password, setPassword, onSubmit }) => (
	<Shell>
		<form onSubmit={(e) => { e.preventDefault(); if (name && email && password) onSubmit(); }}>
			...
			<p className="text-[#EA580C] text-sm font-medium bg-[#1E2025] border border-[#2C2F36] p-3 rounded-lg mb-6">
			...
			<DarkInput label="Password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Your password" />
			<div className="mt-6">
				<BlueBtn type="submit">Create account</BlueBtn>
			</div>
...
const SignUp = () => {
	...
	switch (step) {
		case 0: return <StepEmail name={name} setName={setName} email={email} setEmail={setEmail} password={password} setPassword={setPassword} onSubmit={handleRegister} />;
